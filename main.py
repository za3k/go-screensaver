...
def set_pixel(im, x, y, rgb):
    """Draws a single pixel at (x,y) in color rgb"""
    r,g,b = rgb
    im[x][y][:] = b,g,r

def draw_square(im, x, y, rgb):
...
def draw_go_stone(im, row, col, is_black):
    x, y = (row+1)*GRID_SPACING, (col+1)*GRID_SPACING
    if is_black:
...
def board_to_image(b):
    ...
    for row_num, row in enumerate(b):
        ...
        for col_num, square in enumerate(row):
            ...
            if square == "black":
                draw_go_stone(im, row_num, col_num, True)
